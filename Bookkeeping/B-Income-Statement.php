...
<html>
    <body>
        ...
        <?php       
                    ...
                    $query = "SELECT  `is_details` FROM `tblincomestatement` WHERE `is_month` = '$month' AND `is_year` = '$year'";    
                    $stmt = $conn->prepare($query);
                    $stmt-> execute();
                    $result = $stmt->get_result();  
                    $details = "NO DATA AVAILABLE";
                    while ($row = $result->fetch_assoc()) { 
                      $details   = $row['is_details'];
                    }
    ...
    <div class="selector-cover">
           <div class="title-wrapper center">
                <h2 style="color: #919094;">INCOME STATEMENT</h2>
        ...
        <div class="box">
            <div class="button-footer">
                <button type="button" data-modal-target="#modal3" class="buttonss views">
                    <span class="button__texts">VIEW INCOME STATEMENT</span>
        ...
        <?php       
                    ...
                    $query = "SELECT `is_description`, `is_amount` FROM `tblincomestatement` WHERE `is_month` = '$month' AND `is_year` = '$year' AND `is_category` = 'INCOME'";    
                    $stmt = $conn->prepare($query);
                    $stmt-> execute();
                    $result = $stmt->get_result();  
...
<?php while ($row = $result->fetch_assoc()) { ?>
  <tr>
  <td><?= $row['is_description'] ?></td>
  <td style="text-align:right;"><strong>₱ </strong><?= number_format($row['is_amount']) ?></td>
  </tr>
<?php } ?>

<?php       
      $query = "SELECT  SUM(is_amount) AS total_amount FROM `tblincomestatement` WHERE `is_month` = '$month' AND `is_year` = '$year' AND `is_category` = 'INCOME'";    
      $stmt = $conn->prepare($query);
      $stmt-> execute();
      $result = $stmt->get_result();  
...
?>  
 ...
 <tr style="background: #2B2B2B;">
      ...
      <?php while ($row = $result->fetch_assoc()) { ?>
      <td style="text-align:right;"><strong>₱ </strong><?= number_format($row['total_amount']) ?></td>
      <?php $totalincome = $row['total_amount']; } ?>
            ...
            <?php       
                    $query = "SELECT `is_description`, `is_amount` FROM `tblincomestatement` WHERE `is_month` = '$month' AND `is_year` = '$year' AND `is_category` = 'EXPENSES'";    
                    $stmt = $conn->prepare($query);
                    $stmt-> execute();
                    $result = $stmt->get_result();  
            ...
            ?>
             ...
             <table class="tables">     
              ...
              <?php       
                    $query = "SELECT  SUM(is_amount) AS total_amount FROM `tblincomestatement` WHERE `is_month` = '$month' AND `is_year` = '$year' AND `is_category` = 'EXPENSES'";    
                    $stmt = $conn->prepare($query);
                    $stmt-> execute();
                    $result = $stmt->get_result();  
              ...
              <tr style="background: #2B2B2B;">
                  ...
                  if($totalincome > $totalexpenses){
                    ...
                    if($sum1 < 0){
                      ...
                      <td style="text-align:right; color:#fff" ><strong>₱ </strong> <?php echo "(".number_format($sum).")"?></td>
                      </tr>
                      <?php
                    }else{
                      ?>
                      <tr style="background:#009879;">
                      <th style="color:#a7ff83; text-align:left;" class="total">
                      <?php echo "NET PROFIT"; ?>
                      </th>
                      <td style="text-align:right; color:#a7ff83" ><strong>₱ </strong> <?= number_format($sum)?></td>
                      </tr>
                  <?php }?>
